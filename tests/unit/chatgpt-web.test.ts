...
test("Reference image upload reports the safe upstream status and error code", async () => {
  reset();
  const m = installMockFetch();
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () =>
    new Response('<?xml version="1.0"?><Error><Code>MissingRequiredHeader</Code></Error>', {
      status: 400,
  ...
  try {
    ...
    assert.match(
      ...
      /ChatGPT Web rejected a reference image upload \(HTTP 400, MissingRequiredHeader\)/
    );
    assert.equal(m.calls.conv, 0, "conversation must not start after a rejected reference upload");
  } finally {
    globalThis.fetch = originalFetch;
    m.restore();
  }
