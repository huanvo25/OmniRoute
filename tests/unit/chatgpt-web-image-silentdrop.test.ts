...
test("handler forwards reference images as ChatGPT multimodal attachments", async () => {
  const url = "/v1/chatgpt-web/image/abcdef0123456789";
...
test("request artifact preserves the complete prompt", () => {
  const marker = "TAIL_VISIBLE_IN_OMNIROUTE_REQUEST_HISTORY";
  const prompt = `${"B".repeat(700)}${marker}`;
  ...
  assert.match(artifact.prompt, new RegExp(`${marker}$`));
});
