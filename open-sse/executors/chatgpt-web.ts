...
function imageDimensions(bytes: Buffer, mimeType: string): { width?: number; height?: number } {
  ...
  // JPEG dimensions live in a Start Of Frame segment. We only need the
  // normal baseline/progressive frame types; absent dimensions are accepted
  // by ChatGPT's attachment API, so malformed images never become an error.
...
// Asset delivery is independent of image generation.  ChatGPT quite often
// accepts the generation, then briefly returns 503/timeouts while its file
// service materialises the attachment. Retrying here is safe: these are GETs
// only and must never create a second ChatGPT image turn.
const IMAGE_RETRIEVAL_ATTEMPTS = 3;
const IMAGE_RETRIEVAL_RETRY_DELAY_MS = 1_000;
...
async function fetchDownloadUrl(endpoint: string, ctx: ResolverContext): Promise<string | null> {
  ...
  for (let attempt = 1; attempt <= IMAGE_RETRIEVAL_ATTEMPTS; attempt++) {
    try {
      ...
      if (response.status === 200) {
        try {
          const parsed = JSON.parse(response.text || "{}") as { download_url?: string };
          if (parsed.download_url) return parsed.download_url;
          ctx.log?.warn?.("CGPT-WEB", `Image download URL response had no URL for ${endpoint}`);
        } catch {
          ctx.log?.warn?.("CGPT-WEB", `Image download URL parse failed for ${endpoint}`);
        }
      } else {
        ctx.log?.warn?.(
          "CGPT-WEB",
          `Image download URL fetch failed (${response.status}) for ${endpoint} (attempt ${attempt}/${IMAGE_RETRIEVAL_ATTEMPTS})`
        );
      }
    } catch (err) {
      // A thrown /files request used to bypass the conversation-attachment
      // fallback entirely, which is exactly the path sediment pointers need.
      ctx.log?.warn?.(
        "CGPT-WEB",
        `Image download URL fetch threw for ${endpoint} (attempt ${attempt}/${IMAGE_RETRIEVAL_ATTEMPTS}): ${err instanceof Error ? err.message : String(err)}`
      );
    }
    if (attempt < IMAGE_RETRIEVAL_ATTEMPTS && !ctx.signal?.aborted) {
      await delayWithAbort(IMAGE_RETRIEVAL_RETRY_DELAY_MS, ctx.signal);
    }
  }
  return null;
}
...
async function imageUrlToCachedImageUrl(
  ...
  let response: TlsFetchResult | null = null;
  for (let attempt = 1; attempt <= IMAGE_RETRIEVAL_ATTEMPTS; attempt++) {
    try {
      const candidate = await tlsFetchChatGpt(signedUrl, {
        method: "GET",
        headers,
        timeoutMs: 60_000,
        signal: ctx.signal,
      ...
      if (candidate.status === 200 && candidate.text) {
        response = candidate;
        break;
      }
      ctx.log?.warn?.(
        "CGPT-WEB",
        `Image fetch returned HTTP ${candidate.status} (${(candidate.text || "").slice(0, 120)}) (attempt ${attempt}/${IMAGE_RETRIEVAL_ATTEMPTS})`
      );
    } catch (err) {
      ctx.log?.warn?.(
        "CGPT-WEB",
        `Image fetch failed (attempt ${attempt}/${IMAGE_RETRIEVAL_ATTEMPTS}): ${err instanceof Error ? err.message : String(err)}`
      );
    }
    if (attempt < IMAGE_RETRIEVAL_ATTEMPTS && !ctx.signal?.aborted) {
      await delayWithAbort(IMAGE_RETRIEVAL_RETRY_DELAY_MS, ctx.signal);
