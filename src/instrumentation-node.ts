...
export async function registerNodejs(): Promise<void> {
  ...
  // Clear only expired cooldowns left by an unclean crash. A future
  // `rate_limited_until` is valid account-limit state and must survive restart.
  try {
    const { clearStaleCrashCooldowns } = await import("@/lib/db/providers");
    const { cleared } = clearStaleCrashCooldowns();
    if (cleared > 0) {
      console.log(`[STARTUP] Cleared ${cleared} expired connection cooldown(s) from prior crash`);
    }
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err);
